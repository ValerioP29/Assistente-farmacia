...
CREATE TABLE jta_pharma_patient (
    ...
    active_flag TINYINT(1)
        AS (CASE WHEN deleted_at IS NULL THEN 1 ELSE 0 END) STORED,
...
CREATE TABLE jta_therapies (
    ...
    CONSTRAINT fk_therapy_pharma  FOREIGN KEY (pharma_id)  REFERENCES jta_pharmas(id)  ON DELETE CASCADE,
    CONSTRAINT fk_therapy_patient FOREIGN KEY (patient_id) REFERENCES jta_patients(id) ON DELETE CASCADE,

    INDEX idx_therapy_pharma (pharma_id),
    INDEX idx_therapy_patient (patient_id),
    INDEX idx_therapy_active (active, start_date)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
